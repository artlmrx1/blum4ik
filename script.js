(() => {
    ...
    const intervalId = setInterval(() => {
        ...
        const amountElement = document.querySelector('div[data-v-3adc5550].amount');
        if (amountElement && amountElement.offsetParent !== null) {
            const amount = amountElement.textContent.trim();
            console.log(`Нафармлено: ${amount}`);
            clearInterval(intervalId);
            delete window.findAndClickObjects;
            delete window.simulateClick;
        }
    }, 100);
